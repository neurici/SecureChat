...
function sendMessage()
{
	...
	if (key == "" || key == "=")
	{
		alert("Lipseste cheia (partea URL a site-ului web de după „#”).");
	}
	else
	{
		...
		if (message != "")
		{
			...
			xhr.onreadystatechange = function()
			{
				if (xhr.readyState == XMLHttpRequest.DONE)
				{
					...
					{
						document.getElementById("chatroom").insertAdjacentHTML("beforeend", "<i>Eroare la trimiterea mesajului</i><br/>");
					}
				}
			}
		...
		if (xhr.readyState == XMLHttpRequest.DONE)
		{
			...
			if (xhr.status === 200)
			{
				...
				if (data && data.messages)
				{
					if (key == "" || key == "=")
					{
						chatRoom.innerHTML = "<i>Lipseste cheia (partea URL a site-ului web de după „#”).</i>";
						stopTimerCheck();
						return;
					}
					...
					if (data.messages.length > 0)
					{
						...
						if (changeTitle && !isRefreshTitle)
						{
							refreshTitleInterval = setInterval(function()
							{
								...
								{
									document.title = "Mesaj nou pe chat!";
								}
								else
								{
			...
			else if (xhr.status === 400)
			{
				if (chatRoom !== null)
				{
					chatRoom.innerHTML = "<i>Aceasta conversatie s-a incheiat... Ar trebui sa începi una noua pentru a continua sa conversezi!</i>";
					stopTimerCheck();
				}
			}
			else if (xhr.status === 403)
			{
				chatRoom.innerHTML = "<i>Aceasta conversatie s-a autodistrus. A fost creata doar pentru doi utilizatori.</i>";
				stopTimerCheck();
			}
		}
	}
...
function removeChatroom(withPassword)
{
	if (confirm('Sunteti sigur?'))
	{
		...
		if (withPassword) {
			var removePassword = prompt("Va rog sa introduceti parola pentru a sterge camera de chat", "");
		}
